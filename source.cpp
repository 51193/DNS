...
void CreateDNSQuery(unsigned char *UserData, unsigned int &UserDataLength, const char *domain_1, const char *domain_2)
{
    ...
    // 这里简化处理，假设domain是"example.com"
    UserData[index++] = strlen(domain_1); // "example"长度
    memcpy(UserData + index, domain_1, strlen(domain_1));
    index += strlen(domain_1);
    UserData[index++] = strlen(domain_2); // "com"长度
...
int main(int argc, char *argv[])
{
    ...
    // 填充地址并生成数据包包头
    char SourceMAC[MAX_PATH] = "70-A6-CC-3E-97-39";
    char SourceIP[MAX_PATH] = "10.129.88.199";
    char SourcePort[MAX_PATH] = "3306";

    char DestinationMAC[MAX_PATH] = "10-4f-58-6c-0c-00";
    char DestinationIP[MAX_PATH] = "114.114.114.114";
    char DestinationPort[MAX_PATH] = "53";

    // 准备DNS查询数据
    unsigned char dnsQuery[MAX_PATH]; // 分配足够的空间来构造DNS请求
    unsigned int dnsQueryLength = 0;
    CreateDNSQuery(dnsQuery, dnsQueryLength, "baidu", "com");
    ...
    GetWindowsTimestamp(&header.ts);     // 使用Windows API获取当前时间
    header.caplen = dnsQueryLength + 42; // 捕获长度
    header.len = header.caplen;          // 数据包的实际长度

    // 将发送的数据包写入pcap文件
    pcap_dump((u_char *)dumpfile, &header, FinalPacket);
    ...
    std::cout << "DNS request has sent" << std::endl;
